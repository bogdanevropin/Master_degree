...
if {[llength [info commands interp]]} {
	...
	if {!$code && [string match c* [lindex $args 0]]} {
		...
		$result eval [list set argv0 $argv0]
            ...
            # Tcl expr fixes and extras.  Note: Oc_AddTclExprExtensions
            # is C-level extension from imports.cc, so won't be available
            # if Oc is loaded as a Tcl-only package.
...
################################
# "Safe" subset of "file" command, for use in safe child interpreters.
# Use as
#   interp alias $slave file {} Oc_SafeFile
proc Oc_SafeFile { args } {
   ...
   switch -exact $subcmd {
      ...
      default {
         error "File subcommand $subcmd not supported in slave interp"
      }
   }
}
