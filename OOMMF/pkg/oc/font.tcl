...
Oc_Class Oc_Font {
	...
	if {[string match windows $tcl_platform(platform)]} {
	    ...
	    set bca "-*-fixed-bold-*-*-*-*-*-*-*-*-*-*-*"
	    if {![catch {$x configure -font $bc}]} {
               # Note 1: This test should always succeed if Tcl is 8.0
               #   or later, because of font-substitution.
               # Note 2: We want this font to be smaller than the
               #   standard font.  If it isn't, try to resize:
               set font(compressed_bold) $bc
               set stdsize [font metrics $font(standard) -linespace]
               set tstsize [font metrics $bc -linespace]
               if {$tstsize>$stdsize && $stdsize>0} {
                  set bcx [format \
                      "-misc-fixed-bold-r-semicondensed-*-*-%d-*-*-*-*-*-*" \
                      [expr {int(round(0.90*120*$stdsize/$tstsize))}]]
                  if {![catch {$x configure -font $bcx}]} {
                     set font(compressed_bold) $bcx
                  }
               }
