...
# Environment variable override for C++ compiler.  Also allow OOMMF_CPP,
# because OOMMF_C++ is an invalid key for some shells.
...
########################################################################
# CUDA
set cuda_compiler {nvcc -c -w -m 64 -DNDEBUG -use_fast_math -Xcompiler \
                  "-O2 -ffast-math -frename-registers -fstrict-aliasing \
                   -fweb -fomit-frame-pointer"}
if {![catch {$config GetValue program_compiler_cuda_local_options} \
         cuda_local_opts]} {
   set cuda_compiler [concat $cuda_compiler $cuda_local_opts]
}
$config SetValue program_compiler_cuda $cuda_compiler
$config SetValue program_compiler_cuda_option_src {format \"%s\"}
...
$config SetValue program_compiler_cuda_option_gencode {format "-gencode \"%s\""}
set cuda_base_path {}
if {[info exists env(CUDA_HOME)]} {
   set cuda_base_path $env(CUDA_HOME)
} elseif {[info exists env(CUDA_PATH)]} {
   set cuda_base_path $env(CUDA_PATH)
} else {
   set tryvars [lsort [array names env CUDA_PATH*]]
   if {[llength $tryvars]>0} {
      # If more than one then which to pick?!
      set cuda_base_path $env([lindex $tryvars end])
   }
}
if {![string match {} $cuda_base_path]} {
   # CUDA include path is a list of directories. Be sure to protect
   # spaces in pathnames.
   $config SetValue program_compiler_cuda_system_include_path \
      [list [file join $cuda_base_path include]]
   # Particular subdirectory for CUDA_LIB_SPEC will vary by platform
   set trypath [file join $cuda_base_path lib64]
   if {![file isdirectory $trypath]} {
      set trypath [file join $cuda_base_path lib x64]
   } elseif {![file isdirectory $trypath]} {
      set trypath [file join $cuda_base_path lib]
      ## Last try
   }
   $config SetValue CUDA_LIB_SPEC $trypath
   $config SetValue CUDA_LIBS cudart   ;# CUDA runtime library
   set cuda_compiler [$config GetValue program_compiler_cuda]
   set ccname [lindex $cuda_compiler 0]
   set ccname [file join $cuda_base_path bin $ccname]
   if {[file executable $ccname]} {
      set cuda_compiler [lreplace $cuda_compiler 0 0 $ccname]
      $config SetValue program_compiler_cuda $cuda_compiler
   }
}
...
#
## CUDA local options
# $config SetValue program_compiler_cuda_local_options "--cudart static"
#
#
...
if {[Oc_Option Get Threads oommf_thread_count _] && \
       [catch {$config GetValue thread_count}]} {
   # Value not set in Oc_Options or platforms/local/linux-x86_64.tcl, so
   # use getconf to report the number of "online" processors.  NOTE:
   # Neither _NPROCESSORS_ONLN nor processor_count in /proc/cpuinfo
   # distinguish between physical cores and logical cores introduced via
   # hyperthreading.
