...
if {[Oc_Option Get Threads oommf_thread_count _] && \
   ...
   # Value not set in Oc_Options or platforms/local/darwin.tcl, so use
   # sysctl or system_profiler to get count of physical cores.  If those
   # fail, then cut auto_max in half and fall back to getconf or sysctl
   # to get count of logical cores (which may include cores introduced
   # by hyperthreading).
