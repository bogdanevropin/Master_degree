 ...
 OC_INDEX node_start,OC_INDEX node_stop) const
{
  ...
  const OC_INDEX txdim = DimX();  // For convenience
  const OC_INDEX tydim = DimY();  // For convenience
  const OC_INDEX tzdim = DimZ();  // For convenience
  const OC_INDEX txydim = txdim*tydim;
  ...
  for(OC_INDEX index=node_start;index<node_stop;++index) {
    if(zero_check[index]!=0.0) {
      if(ix+1<txdim && zero_check[index+1]!=0.0) {
        OxsRectangularMeshAngle test(vec[index],vec[index+1]);
        if(test>maxangX) maxangX = test;
      }
      if(iy+1<tydim && zero_check[index+txdim]!=0.0) {
        OxsRectangularMeshAngle test(vec[index],vec[index+txdim]);
        if(test>maxangY) maxangY = test;
      }
      if(iz+1<tzdim && zero_check[index+txydim]!=0.0) {
        OxsRectangularMeshAngle test(vec[index],vec[index+txydim]);
 ...
 OC_INDEX node_start,OC_INDEX node_stop) const
{
  ...
  const OC_INDEX tzdim = DimZ();  // For convenience
  const OC_INDEX txydim = txdim*tydim;
  const OC_INDEX zwrap = (tzdim-1)*txydim;
  ...
  for(OC_INDEX index=node_start;index<node_stop;++index) {
    if(zero_check[index]!=0.0) {
      ...
      } else if(xperiodic && zero_check[index+1-txdim]!=0) {
        OxsRectangularMeshAngle test(vec[index],vec[index+1-txdim]);
        if(test>maxangX) maxangX = test;
      }
      if(iy+1<tydim) {
        if(zero_check[index+txdim]!=0.0) {
          OxsRectangularMeshAngle test(vec[index],vec[index+txdim]);
          if(test>maxangY) maxangY = test;
        }
      } else if(yperiodic && zero_check[index+txdim-txydim]!=0) {
        OxsRectangularMeshAngle test(vec[index],vec[index+txdim-txydim]);
        if(test>maxangY) maxangY = test;
      }
      if(iz+1<tzdim) {
        if(zero_check[index+txydim]!=0.0) {
          OxsRectangularMeshAngle test(vec[index],vec[index+txydim]);
          if(test>maxangZ) maxangZ = test;
        }
