...
Oc_Class Platform {
    ...
    public common cuflags {}   ;# For CUDA
...
#    handle unmatched open brace as an element.  The replacement is safe.
    ...
    proc LinkerLibs {stemlist} {
       ...
       foreach stem $stemlist {
          ...
          if {[regexp {^cu..} $stem] && \
                 ![catch {$configuration GetValue CUDA_LIB_SPEC} libspec]} {
             # Assume this stem refers to a CUDA library                                           
             set lf {}
             set script [$configuration GetValue script_filename_static_library]
             if {[string length $script]} {
                set lf [file join $libspec [eval $script $stem]]
             }
             set ret [concat $ret [$class LinkTemplate $lf $stem]]
             continue
          }
          # Otherwise assume it is a static library under oommf/pkg/                               
    ...
    proc Compile {language args} {
       ...
       if {[string match C++ $language]} {
          set args [concat $cflags $args]
       } elseif {[string match CUDA $language]} {
          set args [concat $cuflags $args]
       }
