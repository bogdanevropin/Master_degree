...
Oc_Class CSourceFile {
    ...
    ClassConstructor {
       ...
       if {![catch {$configuration GetValue \
                ...
                cpppath]} {
          set sysdeppath [concat $sysdeppath $cpppath]
       }
       # NOTE: sysdeppath are directories that are built into the
       # compiler and so don't need to be explicitly listed in the
       # compile command.  Moreover, some compilers complain if these
       # directories do appear explicitly in the compile command, so
       # such directories are actively expunged from the compile
       # command.
